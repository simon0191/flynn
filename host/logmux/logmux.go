...
func (m *Mux) addSink(sink Sink) {
	...
	// TODO(jpg) need to way to delay resuming the normal loop after an error
	// to prevent spinning forever. exponential backoff is probably what we want
	for {
		select {
		case <-shutdownCh:
			sink.Close()
		...
		default:
			func() {
				...
				//TODO(jpg) this logic belongs in the LogAggregatorSink
				// the interface should be changed to GetCursor()
